...
char* comparateurReverse(char cmp[]){
    ...
    if(strcmp(cmp,"<") == 0)   {return "BGE";}
    if(strcmp(cmp,"==")==0)  {return "BNE";}
    if(strcmp(cmp,"!=")==0)  {return "BE";}
    if(strcmp(cmp,"<=")==0)  {return "BG";}
    if(strcmp(cmp,">=")==0)  {return "BL";}
    if(strcmp(cmp,">")==0)   {return "BLE";}
    strcpy(cmp,""); return "";
...
int DoubleDeclaration(char *entite){
    ...
    if(t != NULL){
        // j'ai pas trouvÈ le type associÈ ‡ l'entitÈ dans le
...
int NotDeclared(char *entite){
    ...
    if((temp != NULL)){//||(temp->nom == NULL)
        if(temp->type != -1){//donc var est declaree
...
int affect_compatibl(int typeOP1,int typeOP2){
      ...
      */
      if(typeOP1 == typeOP2) return 1;

      if((typeOP1 == 1)&&(typeOP2 == 0))
          return 1;
      if((typeOP1 == -1)||(typeOP2 == -1)) return 1;
...
int indexOutOfBounds(char *entite,int index){

      Node *temp = recherche(entite);
      if(temp != NULL){
            if((index > temp->taille)||(index < 0)){ 
...
initializeEntier(int *OP1,int *OP2){
    (*OP1) = -1;
    (*OP2) = -1;
}